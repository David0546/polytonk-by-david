    ...
    const loop = function() {
        ...
        if (config.gameState == config.gameStates.ingame || config.gameState == config.gameStates.goingInGame || config.gameState == config.gameStates.menu || config.gameState == config.gameStates.goingToMenuFromDeath || config.gameState == config.gameStates.account) {
            ...
            if (player.showingTank) {
                ...
                ctx.fillStyle = colors[0];
                ctx.globalAlpha = 1;
                util.drawText(ctx, "kanono template", config.screenWidth / 8, config.screenHeight / 8 - 70, 20)
        ...
        if (config.gameState == config.gameStates.menu || config.gameState == config.gameStates.goingInGame || config.gameState == config.gameStates.goingToMenuFromDeath) {
            ...
            ctx.globalAlpha = 1;
            util.drawText(ctx, "kanono template" + (window.beta ? " (private)" : ""), config.screenWidth / 2, (config.screenHeight / 2 - 100 - animations.menu * 400) - animations.menuSlide * config.screenHeight / 1.5, 60, "center");
            ...
            if (config.accountsEnabled) {
                ...
                ctx.fill();
                ctx.fillStyle = colors[0];
                util.drawText(ctx, "Kanono accounts (Beta)", config.screenWidth / 2, config.screenHeight - config.spacing - 12.5 - animations.menu * 30 + animations.menuSlide * 100, 20 + animations.loginButton * 2.5, "center");
            } else {
                ctx.fillStyle = colors[0];
                util.drawText(ctx, "Sorry, kanono accounts are disabled in this server.", config.screenWidth / 2, config.screenHeight - config.spacing - 12.5 - animations.menu * 30 + animations.menuSlide * 100, 20, "center");
            };
