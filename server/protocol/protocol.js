...
exports.encode = function (...stuff) {
  ...
  switch (stuff[0]) {
    case "roomInfo": // Room info
      ...
      buf.put_u8(stuff[4]); // Accounts enabled
      break;
    ...
    case "entities": // Entities
      ...
      for (let key in stuff[1]) { // the u32 we got is how many entities the server sent to us.
        ...
        buf.put_float(stuff[1][key].health);
        buf.put_u8(stuff[1][key].barrelFlash);
        buf.put_u8(stuff[1][key].shieldFlash);
      };
      break;
    case "goingInGame":
      buf.put_u8(3);
    ...
    case "account":
            ...
            if (!stuff[1]) {
                ...
                buf.put_u16(stuff[3]);
                buf.put_u8(stuff[4]);
                buf.put_utf8(stuff[5]);
                buf.put_utf8(stuff[6]);
                buf.put_u16(stuff[7]);
                buf.put_u32(stuff[8]);
                buf.put_u32(stuff[9]);
            };
    ...
    case "dominationColors":
      buf.put_u8(15);
      buf.put_u8(stuff[1][0]);
      buf.put_u8(stuff[1][1]);
      buf.put_u8(stuff[1][2]);
      buf.put_u8(stuff[1][3]);
  };
  return buf.buffer;
  } catch(e) {};
};
...
exports.decode = function (...stuff) {
  ...
  switch (type) {
    ...
    case 7:
      output.push("login");
      output.push(buf.get_u8(stuff[1])); // register(0) or login(1)
      output.push(buf.get_utf8(stuff[2])); // name
      output.push(buf.get_utf8(stuff[3])); // password
      break;
    default:
      return ["unknown"];
  };
